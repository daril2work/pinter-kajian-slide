...
const Header: React.FC = () => {
  ...
  const appTitle = heroContent?.title || "Takmir Pinter";
  ...
  return (
    <header className="bg-white/90 backdrop-blur-sm border-b border-emerald-100 sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-4">
        <div className="flex items-center justify-between">
          {/* Logo and Title - Now using dynamic content */}
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-emerald-600 rounded-lg flex items-center justify-center">
              <span className="text-white font-bold text-lg">🕌</span>
            </div>
            <div>
              <h1 className="text-xl font-bold text-gray-900">
                {appTitle.includes("Pinter") ? (
                  <>
                    {appTitle.split("Pinter")[0]}
                    <span className="text-emerald-600">Pinter</span>
                    {appTitle.split("Pinter")[1]}
                  </>
                ) : (
                  <>
                    {appTitle} <span className="text-emerald-600">Pinter</span>
                  </>
                )}
              </h1>
